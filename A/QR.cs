namespace A
{
    ...
    public class QR
    {
        ...
        public static byte[] A(string A) => 
            QR.A.ComputeHash(Encoding.ASCII.GetBytes(A));

        public Image A(string A, string R)
        {
            int num;
            int num2;
            string expression = "";
            ...
            expression = expression + "0011100010100";
            Bitmap image = new Bitmap(Strings.Len(expression), 60, PixelFormat.Format24bppRgb);
            ...
            graphics.DrawString(A, new Font("times new roman", 12f, FontStyle.Bold), new SolidBrush(Color.DarkBlue), (float) 20f, (float) 40f);
            return image;
        }
        ...
        {
            QR.A.Key = QR.A(R);
            QR.A.Mode = CipherMode.ECB;
            byte[] bytes = Encoding.ASCII.GetBytes(A);
            return Convert.ToBase64String(QR.A.CreateEncryptor().TransformFinalBlock(bytes, 0, bytes.Length));
        }

        public Image R(string A, string R)
        {
            int num;
            int num2;
            string expression = "";
        ...
        public static string R(string A, string R)
        {
            try
            {
                ...
                return Encoding.ASCII.GetString(QR.A.CreateDecryptor().TransformFinalBlock(inputBuffer, 0, inputBuffer.Length));
            }
